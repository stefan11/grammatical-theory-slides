...
[S\sub{$+$SUB}
  [{NP[nom]}]
  [{S\sub{$+$SUB}$\backslash$NP[nom]}
    [{NP[acc]}]
    [{(S\sub{$+$SUB}$\backslash$NP[nom])$\backslash$NP[acc]}]]]
\end{forest}
}  
\hfill\mbox{}
\medskip
...
The category np can be transformed into the category (s/(s\bs np)) by \emph{type raising}. If we
combine this category with (s\bs np), then we get the same result as if we had combined np and (s\bs
np) with the backward application rule.
...
\ea
 Forward composition (> B)\\
    \gruen<2>{X/\gruen<5>{Y}} $*$ \gruen<3,5>{Y}/\gruen<4>{Z} = \gruen<5>{X/Z} 
\z
...
\pause
\item If I combine X/Y with Y/Z despite the missing Z,\\
...
Assuming that the relative pronoun is the head is problematic since so-called pied piping as in
(\mex{1}) cannot be explained straighforwardly \citep{Pollard88a}.
